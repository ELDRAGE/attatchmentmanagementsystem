...
$student_fname = isset($_COOKIE["student_first_name"]) ? $_COOKIE["student_first_name"] : '';
$student_lname = isset($_COOKIE["student_last_name"]) ? $_COOKIE["student_last_name"] : '';
$student_full_name = $student_fname . " " . $student_lname;
$student_index_number = isset($_COOKIE["student_index_number"]) ? $_COOKIE["student_index_number"] : '';

// Check if record exists
$check_content_existence = "SELECT * FROM week1_table WHERE index_number='$student_index_number'";
...
if (isset($_POST["btn_save"])) {
    ...
    if ($monday_job_assigned != "" && $monday_skill_acquired != "" &&
...
$get_previous_data = "SELECT * FROM week1_table WHERE index_number='$student_index_number'";
$execute_get_query = mysqli_query($conn, $get_previous_data);

if (!$execute_get_query) {
    die("Query failed: " . mysqli_error($conn));
}
...
if (!$get_data) {
    // If no data is found, initialize variables
...
<ul id="menu_list">
  ...
  <a class="menu_items_link" href="elogbook.php"><li class="menu_items_list" style="background-color:orange;padding-left:16px">E-Logbook</li></a>
      ...
      <div class="panel-body pbody">
        <div id="navigation_holder">
          <ul class = "pager">
             ...
             <li class = "next"><a href = "elogbook_week2.php">Next &rarr;</a></li>
        </ul>
        </div>
        <div id="week_holder">
          <span style="font-size:1.3em;font-weight:bold;">Week 1</span>
        </div>
        <hr>
...
if ($get_data) {
    ...
    // If no data is found, initialize variables with empty strings
    $monday_job_assigned_holder = '';
    $monday_skill_acquired_holder = '';
